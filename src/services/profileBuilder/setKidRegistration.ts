...
export function setKidRegistration(params: any) {
  const url =
    API_URL_CONSTANTS.BASE_URL + API_URL_CONSTANTS.SET_KID_REGISTRATION;
  return (dispatch: any) => {
    dispatch({
      type: actionTypes.SET_KID_REGISTRATION_SUCCESS_ACTION,
    ...
    postReq(url, params)
      .then((response: any) => {
        if (response.status === 200 && response.data.status === 1) {
          ...
          dispatch({
            type: actionTypes.SET_KID_REGISTRATION_FAILURE_ACTION,
